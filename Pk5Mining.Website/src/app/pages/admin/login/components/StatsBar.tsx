...
export function StatsBar({ stats }: StatsBarProps) {
  return (
    <div className="absolute left-[47px] bottom-[5%] w-[657.5px] h-[117px] bg-black/45 rounded-[18px] z-20 flex items-center overflow-hidden px-8">
      <div className="grid grid-cols-3 w-full h-[85.5px]">
        {stats.map((stat, idx) => (
          <div
            key={idx}
            className={`flex flex-col justify-center items-start pl-8 ${
              idx !== stats.length - 1 ? "border-r border-[#C89B3C]" : ""
            }`}
          >
